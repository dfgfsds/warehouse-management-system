...
export const Administration: React.FC = () => {
  ...
  const [deleteWareModal, setDeleteWareModal] = useState(false);
  ...
  const handleDeleteWarehouse = async () => {
    try {
      const response = await axios.delete(`${baseUrl?.vendors}/${user?.vendor_id}/hubs/${deleteWarehouse?.id}`)
      if (response) {
        setDeleteWareModal(false);
        getWarehouses();
      }
    } catch (error) {
  ...
  return (
    <div className="p-6 space-y-6">
                  ...
                  <div key={warehouse?.id} className="bg-gray-50 p-4 rounded-lg border border-gray-200">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex space-x-1">
                        {/* <button
                          onClick={() => {
                            setDeleteWareModal(!deleteWareModal)
      ...
      <DeleteConfirmModal
        open={deleteWareModal}
        title="Delete wareHouse"
        description={`Are you sure you want to delete "${deleteWarehouse?.name}"?`}
        onCancel={() => setDeleteWareModal(false)}
